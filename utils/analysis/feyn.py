...
class Model():
    ...
    def init_particles(self, sixb):
        btag_mask = ak.argsort(sixb.jet_btag, axis=1, ascending=False) < 6

        pt = sixb.jet_ptRegressed[btag_mask][self.combos]
        phi = sixb.jet_phi[btag_mask][self.combos]
        eta = sixb.jet_eta[btag_mask][self.combos]
        m = sixb.jet_mRegressed[btag_mask][self.combos]
        btag = sixb.jet_btag[btag_mask][self.combos]
        sig_id = sixb.jet_signalId[btag_mask][self.combos]
        h_id = (sixb.jet_signalId[btag_mask][self.combos] + 2) // 2

        self.btag_avg = ak.mean(btag, axis=1)
        setattr(sixb, 'btag_avg', self.btag_avg)
        ...
        self.Y = Y(H1, H2)
        setattr(sixb, 'Y', self.Y)

        self.H1 = self.Y.H1
        self.H2 = self.Y.H2
        setattr(sixb, 'H1', self.H1)
        setattr(sixb, 'H2', self.H2)

        assert ak.all(self.H1.pt >= self.H2.pt)

        self.X = self.HX + self.H1 + self.H2
        setattr(sixb, 'X', self.X)
