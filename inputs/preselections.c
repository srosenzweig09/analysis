...
int preselections(){
  ...
  TFile *fout = new TFile("NanoAOD/NMSSM_XYH_YToHH_6b_MX_700_MY_400_reco_preselections.root","RECREATE");
  TTree *t1   = new TTree("sixBtree","sixBtree");

  TString tree = "sixBtree";
  TChain *cc  = new TChain(tree);

  TString file1 = "NanoAOD/NMSSM_XYH_YToHH_6b_MX_700_MY_400_accstudies.root";
  TString file2 = "NanoAOD/NMSSM_XYH_YToHH_6b_MX_700_MY_400_accstudies_2.root";
  ...
  while(reader.Next()){
    ...
    if (*gen_HX_b2_recojet_pt < pt_threshold) continue;
    if (*gen_HY1_b1_recojet_pt < pt_threshold) continue;
    if (*gen_HY1_b2_recojet_pt < pt_threshold) continue;
    if (*gen_HY2_b1_recojet_pt < pt_threshold) continue;
    if (*gen_HY2_b2_recojet_pt < pt_threshold) continue;
