...
if args.type == 'reco':
    examples = np.load(f'inputs/reco/nn_input_MX700_MY400_{args.task}.npz')
...
scores = scores[:,0]
np.savez(f'models/{args.task}/{args.type}/scores_{args.run}.npz', scores=scores, target=y_test)
